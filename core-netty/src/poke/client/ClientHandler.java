...
public class ClientHandler extends SimpleChannelUpstreamHandler {
	...
	public boolean send(GeneratedMessage msg) {
		...
		//msg = appendChannel(msg,cf);
		ChannelFuture cf = channel.write(msg);
	
		int temp = ((InetSocketAddress)cf.getChannel().getLocalAddress()).getPort();
		String clientPort = temp+"";
	...
	public GeneratedMessage appendChannel(GeneratedMessage msg, String clientPort){
		Request.Builder reqB = Request.newBuilder();
		Request  request = (Request)msg;
		eye.Comm.Header.Builder h = Header.newBuilder();

		Finger.Builder f = eye.Comm.Finger.newBuilder();
		f.setTag(request.getBody().getFinger().getTag());
		f.setNumber(1);
		...
		h.setReplyMsg("Coming");
		h.setFirstChannel(clientPort);
		reqB.setHeader(h.build());
		request = reqB.build();
		return (GeneratedMessage)request;
