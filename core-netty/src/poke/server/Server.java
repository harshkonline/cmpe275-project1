...
public class Server {
	...
	Thread EdgeLinker;
	public static HashMap<String,Boolean> availability = new HashMap<String,Boolean>();

	public static HashMap<String, Boolean> getAvailability() {
		return availability;
	}

	public static void setAvailability(HashMap<String, Boolean> availability) {
	...
	protected void run() {
		...
		// start heartbeat
		final String nodeId = conf.getServer().getProperty("node.id");
		heartbeat = ServerHeartbeat.getInstance(nodeId);
		heartbeat.start();
			...
			public void run() {
				if (null != conf.getNodes() && conf.getNodes().size() > 0) {
					...
					while (itr.hasNext()) {
							...
							managePort = node.getMgmt();	
							//availibility.put(node.getId(), false);
							new Thread(new Runnable() {
								String args[] = { "localhost", managePort ,nodeId};
		...
		if (null != conf.getNodes() && conf.getNodes().size() > 0) {
			...
			while (itr.hasNext()) {
					ServerConf.NodeConf node = (ServerConf.NodeConf) itr.next();
					String managePort = node.getMgmt();
					boolean res = false;
					int tries = 0;
					while (tries < 10 )
					{
