...
public class ResourceUtil {
	...
	public static Header buildHeaderFrom(Header reqHeader, ReplyStatus status,
			String statusMsg,String destination, String route) {
		return buildHeader(reqHeader.getRoutingId(), status, statusMsg,
				reqHeader.getOriginator(), reqHeader.getTag(), reqHeader.getDestination(),reqHeader.getRoutingPath(),reqHeader.getMessageType());
	}

	public static Header buildHeader(Routing path, ReplyStatus status,
			String msg, String from, String tag, String destination, String route, String message_type) {
		Header.Builder bldr = Header.newBuilder();
		bldr.setOriginator(from);
		bldr.setRoutingId(path);
		bldr.setTag(tag);
		bldr.setReplyCode(status);
		bldr.setDestination(destination);
		bldr.setRoutingPath(route);
		bldr.setMessageType(message_type);
		if (msg != null)
